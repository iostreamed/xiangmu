		...
		<div class="am-panel am-panel-primary am-margin-horizontal-xl am-margin-vertical">
		  ...
		  <div class="am-scrollable-horizontal">
				  <table class="am-table am-text-nowrap am-table-centered am-table-main">
				    <tr><td>用户账号</td><td>${user.userName }</td></tr>
				    <tr><td>用户昵称</td><td>${user.nikeName }</td></tr>
				    <tr><td>用户类型</td><td><c:choose><c:when test="${user.userType == 1 }">系统管理员</c:when><c:when test="${user.userType == 2 }">经理</c:when><c:otherwise>普通用户</c:otherwise> </c:choose></td></tr>
				  </table>
				</div>
		</div>
  </div>
