...
<body>
	...
	<div id="wrapper">
		...
		<div id="page-wrapper">
			...
			<a href="#" class="btn btn-primary" data-toggle="modal" data-target="#customerAddDialog" >新增</a>
			<a href="javascript:;" class="btn btn-danger" onclick="deleteCustomer()">批量删除</a>
			<a href="${pageContext.request.contextPath}/customer/export.action" class="btn btn-primary">导出Excel</a>
						...
						<table class="table table-bordered table-striped">
							...
							<tbody>
								<c:forEach items="${page.rows}" var="row">
									<tr>
										<td><input type="checkbox" value="${row.cust_id}" name="id"/></td>
										<td>${row.cust_id}</td>
										<td>${row.cust_name}</td>
	...
	<div class="modal fade" id="customerAddDialog" tabindex="-1" role="dialog"
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
					<button type="submit" class="btn btn-primary" onclick="addCustomer()">保存</button>
				</div>
			</div>
		</div>
	</div>
	<div class="modal fade" id="ExcelAddDialog" tabindex="-1" role="dialog"
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				...
				<div class="modal-body"> 
					<form class="form-horizontal" action="${pageContext.request.contextPath}/customer/import.action" method="post" enctype="multipart/form-data">
						<div class="form-group">
	...
	<script type="text/javascript">	
		...
		function addCustomer(){
			$.post("<%=basePath%>customer/insert.action",$("#add_customer_form").serialize(),function(data){
				alert("客户信息添加成功！");
				window.location.reload();
			});		
		}
